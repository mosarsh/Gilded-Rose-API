...
Database with the name “GildedRose” will be created. You will see tables are created and initial data populated for Inventory, InventoryType tables.
...
Authentication to the API is performed via JWT bearer token (JSON Web Token). Technically, JWT uses signature which is signed from headers and claims with security algorithm specified in the headers (example: HMACSHA256). Therefore, JWT is required to be transferred over HTTPs if you store any sensitive information in claims.
...
Call below method will let us know if user is Authenticated or not.
...
Set as StartUp project <>.Test project 
